...
namespace EditeurCarteProjet2
{
    class Carte
    {
        ...
        public void Update(MouseState _mouseState, Vector2 _camera, Char _tileSelected, GameTime _gameTime)
        {
            _tileHover = setTileHover(new Vector2(_mouseState.X, _mouseState.Y));
            this._camera = _camera;

            if (_mouseState.LeftButton == ButtonState.Pressed)
            {
                if ((_tileHover.X < 10 && _tileHover.X >= 0) && (_tileHover.Y < 10 && _tileHover.Y >= 0))
                _tileArray[(int)_tileHover.X, (int)_tileHover.Y] = _tileSelected;
            }
        }
        ...
        public Vector2 setTileHover(Vector2 _positionSouris)
        {
            ...
            if ((_tileHoverAux.X <= 11 && _tileHoverAux.X >= 0) && (_tileHoverAux.Y <= 11 && _tileHoverAux.Y >= 0))
